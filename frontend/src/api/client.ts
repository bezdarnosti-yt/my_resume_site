/* ============================================
   Typed API client — fetch wrapper for FastAPI
   ============================================ */

function resolveBase(): string {
  const raw = (import.meta.env.VITE_API_URL ?? "").trim();
  // Guard against unsubstituted placeholders or empty values in .env
  if (!raw || raw.includes("${")) {
    return "/api/v1";
  }
  return raw.replace(/\/$/, "");
...
/* ---------- Schema mirrors (replace with openapi-typescript output) ---------- */
